...
local function builtin_item_stopped(self)
	...
	if core.get_item_group(item, "sapling") == 0 then
	...
	if core.is_protected(pos, ":nobody") then
		return -- The area is protected by someone
	end

	local node_below = core.get_node(pos_below)
	if core.get_item_group(node_below.name, "soil") == 0 then
		return -- Not soil
	end

	if core.find_node_near(pos_below, 3, {"group:sapling", "group:tree"}) then
	...
	local nodedef = core.registered_nodes[node] or {}
	if not nodedef.buildable_to then
		return
	end
	core.set_node(pos, {name = item})
	self.itemstring = ""
	self.object:remove()
end
...
local entity_def = core.registered_entities["__builtin:item"]
